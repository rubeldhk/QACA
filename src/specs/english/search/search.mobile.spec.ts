...
test.describe("Search Mobile", () => {
  ...
  for (const scenarioKey of testScenarios) {
    const scenario = getDataSet("search.mobile", "searchTestData", scenarioKey);
    ...
    `, async ({ searchPage, commonUtils }) => {
      ...
      await test.step("🔍 AND the user searches for the product", async () => {
        await searchPage.openMobileSearchIcon();
        await searchPage.searchForProductOnMobile(scenario.ProductName);
      });
