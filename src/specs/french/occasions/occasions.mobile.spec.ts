...
test.describe("Page Occasions Mobile - FR", () => {
  ...
  test(`Test case: '${scenario1.testCaseData.testCase}'
    ...
    await test.step("AND the user opens the mobile menu bar", async () => {
      await homepagePage.clickMobileMenuBar();
    });

    await test.step("AND the user verifies occasions dropdown values match expected (mobile)", async () => {
      await occasionsPage.verifyAllOccasionsDropdownValuesMobile(
  ...
  occasionsTestCases.forEach((testCaseId) => {
    const scenario = getDataSet("occasions.mobile", "occasionsTestData", testCaseId);
  ...
  test(`Test case: '${scenario3.testCaseData.testCase}'
    ...
    await test.step("WHEN the user navigates to Occasions page", async () => {
      await occasionsPage.verifyOccasionsPageElementesMobile(scenario3.expectedPageHeadings!);
    });
  });
});
