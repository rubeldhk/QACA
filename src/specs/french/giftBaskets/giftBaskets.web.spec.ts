...
test.describe("Gift Baskets Page", () => {
  ...
  test(`Test case: '${scenario1.testCaseData.testCase}'
    ...
    await test.step("AND the user verifies the Gift Baskets dropdown is visible", async () => {
      await giftBasketsPage.openGiftBasketsDropdown(true);
    });

    await test.step("AND the user verifies all Gift Baskets dropdown values", async () => {
      await giftBasketsPage.verifyAllGiftBasketsDropdownValues();
    });

    await test.step("AND the user verifies Gift Baskets dropdown values match expected", async () => {
      await giftBasketsPage.verifyGiftBasketsDropdownValuesMatch(
